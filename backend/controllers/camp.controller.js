...
const getCamps = asyncHandler(async (req, res) => {
  if (mongoose.connection.readyState === 1) {
    const camps = await Camp.find().sort({ startDate: -1 }).populate('createdBy', 'name role');
    const campsWithStats = await Promise.all(
      camps.map(async (camp) => {
        const screenedCount = await Screening.countDocuments({ screeningCamp: camp._id });
  ...
  const campsWithStats = devStore.camps.map((camp) => {
    const screenings = devStore.screenings.filter((s) => String(s.screeningCamp?._id || s.screeningCamp) === String(camp._id));
...
const getCampById = asyncHandler(async (req, res, next) => {
  ...
  if (mongoose.connection.readyState === 1) {
    const camp = await Camp.findById(id).populate('createdBy', 'name role organization');
    if (!camp) return next(new ApiError(404, 'Camp not found'));

    const screenings = await Screening.find({ screeningCamp: camp._id })
      .sort({ createdAt: -1 })
  ...
  const camp = devStore.camps.find((c) => c._id === id);
  if (!camp) return next(new ApiError(404, 'Camp not found'));
...
const updateCamp = asyncHandler(async (req, res, next) => {
  ...
  const idx = devStore.camps.findIndex((c) => c._id === req.params.id);
